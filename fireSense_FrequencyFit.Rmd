...
----|:--------------|---------------------------------------------------------------------
`formula`|`~1`|a formula describing the model to be fitted. Piece-wised terms can be specifed using `pw(variableName, knotName)`.
...
model <- n_fires ~ -1 + # Remove the intercept
                   ...
                   PcDisturbed:MonthlyDroughtCode_July + PC_OTHER:MonthlyDroughtCode_July +
