...
`nIterDEoptim`|`2000`|integer defining the maximum number of iterations allowed (DEoptim optimizer).
`nIterNlminb`|`500`|if start is not supplied, `nIterNlminb` defines the number of trials, or searches, to be performed by the nlminb optimizer in order to find the best solution.
